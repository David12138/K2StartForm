...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 取消沟通 介入
    /// </summary>
    public class PortalCancelCommunicateInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormCancelCommunicateBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormCancelCommunicateAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.CancelCommunicate.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
