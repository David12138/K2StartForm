...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 撤回 介入
    /// </summary>
    public class WithdrawnInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormWithdrawnBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormWithdrawnAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.Withdrawn.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
