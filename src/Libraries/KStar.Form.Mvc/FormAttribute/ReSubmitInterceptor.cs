...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 重新提交 介入
    /// </summary>
    public class ReSubmitInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormReSubmitBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormReSubmitAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.ReSubmit.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
