...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 拒绝 介入
    /// </summary>
    public class PortalRefusedInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormRefusedBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormRefusedAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.Refused.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
