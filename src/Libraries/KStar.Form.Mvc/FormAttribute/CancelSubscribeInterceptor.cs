...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 取消订阅
    /// </summary>
    public class CancelSubscribeInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormCancelSubscribeBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormCancelSubscribeAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.CancelSubscribe.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
                Folio = model.FormInstance.Folio,
                ProcessCode = model.FormInstance.ProcessCode,
                ProcessName = model.FormInstance.ProcessName,
                Type = (byte)Domain.Logger.UserOperationEnum.Operation,
