...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 转办 介入
    /// </summary>
    public class PortalRedirectInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormRedirectBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormRedirectAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.Redirect.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
