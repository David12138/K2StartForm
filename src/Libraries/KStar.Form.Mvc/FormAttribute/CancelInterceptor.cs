...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 取消流程 介入
    /// </summary>
    class CancelInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormCancelBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormCancelAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.Cancel.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
