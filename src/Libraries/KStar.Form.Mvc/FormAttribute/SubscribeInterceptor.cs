...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 订阅 介入
    /// </summary>
    public class SubscribeInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormSubscribeBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormSubscribeAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.Subscribe.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
