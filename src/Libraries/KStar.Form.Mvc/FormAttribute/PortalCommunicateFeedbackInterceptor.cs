...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 沟通反馈 介入
    /// </summary>
    public class PortalCommunicateFeedbackInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormCommunicateFeedbackBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormCommunicateFeedbackAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.CommunicateFeedback.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
