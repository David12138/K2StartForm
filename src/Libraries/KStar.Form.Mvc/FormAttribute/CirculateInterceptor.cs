...
namespace KStar.Form.Mvc.FormAttribute
{
    ...
    /// 传阅 介入
    /// </summary>
    public class CirculateInterceptor : PointcutAttribute
    {
        public override async Task OnInvocation(AspectContext aspectContext, AspectDelegate _next)
        {
            ...
            if (isOk)
            {
                ...
                service.OnFormCirculateBefore(model);//执行前
            }
            ...
            if (isOk)
            {
                ...
                service.OnFormCirculateAfter(model);//执行后
            }
            ...
            Domain.Logger.DbLogManager.Post(System.Web.HttpContext.Current, new Domain.Logger.PrcServer_UserOperationLog()
            {
                ...
                ApprovalType = string.IsNullOrEmpty(model.Operation.ActionName) ? Platform.WorkFlow.Models.ActionType.Circulate.GetDescription() : model.Operation.ActionName,
                StartTime = startTime,
