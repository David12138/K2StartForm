...
//获取多语言
function getVueLang(lang, vueInstance) {
    if (lang == "zh-CN") {
        vueInstance._i18n.locale = "cn";
        ELEMENT.locale(ELEMENT.lang.zhCN);
    }
    else if (lang == "en-US") {
        vueInstance._i18n.locale = "en";
        ELEMENT.locale(ELEMENT.lang.en);
    }
    //本地缓存设置语言类型 中文(cn),英文（en）
    localStorage.Lang = vueInstance._i18n.locale;

    return vueInstance._i18n.locale;
}
//获取多语言内容
...
KStarForm = {
                            ...
                            _.each(actNames, function (act) {
                                ...
                                if (userAct) {
                                    ...
                                    if (userAct.ProcessingSource == 2 || userAct.ProcessingSource == 3) {
                                        ...
                                        KStarForm.vmFormData.vmOperation.ManualUsers = _.reject(KStarForm.vmFormData.vmOperation.ManualUsers, { ActivityName: item.ActivityName });
                                    }
                                }
                                else {
                                    //如果不存在，就删除手选数据
    ...
    applyData: function (sender) {
        var kstarFromVMIntance = sender;
        var queryObj = this.getQuerymodel();
        var url = getControllerName() + "/GetFormData" + geturlParam();

        // 使用同步Ajax请求，使得后台定义的vmFormContent数据Model的属性在页面绑定时不会出错
        ajaxSync(url, 'POST', queryObj, function (response) {
            var jsonData = JSON.parse(response);

            // 转换后台的多语言格式为前端可用的格式
            jsonData.UserLanguages = getVueLang(jsonData.UserLanguages, kstarFromVMIntance);

            //绑定vue前端
            KStarForm.formModel = jsonData;
            KStarForm.initFormData(kstarFromVMIntance);
        }, function (error) {
            KStarForm.error(error, kstarFromVMIntance);
        });
    },
    //绑定
