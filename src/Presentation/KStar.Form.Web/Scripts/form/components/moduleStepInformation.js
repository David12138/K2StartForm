...
var templateContent = '\
    ...
    <mobileuserpick :pop-visible="parmdialog.mobiledialogvisible" :multiple="parmdialog.multiplelimit" :title="parmdialog.title" :params="parmdialog"  v-on:closedialog = "closedialog" v-on:change="userpickCallback" v-if="parmdialog.mobiledialogvisible" :type="parmdialog.clicktype"></mobileuserpick>\
        ...
        <el-table size="mini" class="step-information m-form-table" :data="procpredictionmodel" border>\
            ...
            <el-table-column prop="name" :label="$t(\'KStarForm.Approver\')">\
                <template slot-scope="scope">\
                    <el-input v-if="scope.row.ProcessingSource!=1" :disabled="true" v-model="scope.row.ApproverName" size="mini" ><el-button slot="append" v-on:click="selectApprove(scope.row)" icon="ic-search"></el-button></el-input>\
                    <div v-else>{{scope.row.ApproverName}}</div>\
                  </template>\
              </el-table-column>\
            </el-table>\
            </div>\
        ...
        showdialog: function (val, multiple, type, activityId, orgId) {
            ...
            switch (val) {
                case 'ApproveUser'://审核人
                    ...
                    this.parmdialog.title = KStarForm.VM.$t("KStarForm.SelectApprovalUser"); //请选择审核人
