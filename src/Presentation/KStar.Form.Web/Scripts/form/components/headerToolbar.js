...
var templateContent = '\
    <header class="header header-toolbar">\
        ...
        <userpick :parmdialog = "parmdialog2" v-on:closedialog = "closedialog2" v-on:requseturl = "userpickCallBack2"></userpick>\
          <dialogmodal :title="$t(\'KStarForm.ProcessSeer\')" :visible="visible" :button="[$t(\'KStarForm.Confirm\')]" v-on:submit="closeprocPredictiondialog" v-on:cancle="closeprocPredictiondialog" v-if="visible">\
            ...
            <el-table :data="procpredictionmodel" v-if="procpredictionmodel" max-height="300" border class="addUser">\
                ...
                <el-table-column prop="ActivityDisplayName" :label="$t(\'KStarForm.StepName\')">\
                    <template slot-scope="scope">\
                        <el-badge :value="ApprovalModeVaule(scope.row.ApprovalMode, scope.row.Approvers)" class="processSeer-badge">{{ scope.row.ActivityDisplayName}}<div v-if="scope.row.Required" style="color:red;float: right;">*</div><div v-else ></div></el-badge>\
                    </template>\
                </el-table-column>\
                <el-table-column prop="name" :label="$t(\'KStarForm.Approver\')">\
                  <template slot-scope="scope">\
                    <el-input v-if="scope.row.ProcessingSource!=1&&(formtype===\'Application\'||formtype===\'Draft\'||formtype===\'ReApproval\'||formtype===\'Approval\')" readonly v-model="scope.row.ApproverName" size="mini"  v-on:click.native="selectApprove(scope.row)"><i slot="suffix" class="iconfont iconrenyuan"  v-on:click="selectApprove(scope.row)" /></i></el-input>\
                    <div v-else>\
            ...
            <div class="header-btnbar custom-reset-styles position-fixed">\
                <div class="center hidden-xs-only">\
                   <el-button  v-if="isProcessSeer" v-on:click="actProcPrediction" class ="btn-other" size="mini" plain>{{ $t(\'KStarForm.ProcessSeer\') }}</el-button>\
                    <el-button v-if="ifBtn(\'Submit\')" id=\"btnSubmit\" v-on:click="actSubmit" :disabled=btnDisabled  size="mini" type="primary" plain>{{ $t(\'KStarForm.Submit\') }}</el-button>\
                ...
                <div>\
                    ...
                    <el-row :gutter="20"  class="header-toolbar-processRelatedCases">\
                        <el-col :span="5" v-if="IsShowRelatedCase">{{ $t(\'KStarForm.RelatedDocuments\') }}：</el-col><el-col :span="19" v-if="ProcessRelatedCases">\
                            <el-row v-for="(item, index) in ProcessRelatedCases" style="text-align:left" :key="index">\
...
var headerToolbar = Vue.extend({
    ...
    props: ['formtype', 'formbtnmodel', 'formbackactivity', 'formoperationmodel', 'formesetting', 'procpredictionmodel', 'switchvaluemodel'],
    data: function () {
        return {
            btnDisabled: false,
            parmdialog: {
    ...
    methods: {
        ...
        circulateSubmit: function () {
            //debugger;
            var that = this
            // if (that.dialogModalParm.actionType == 'Approve') {
            ...
            this.$refs['formModelRef'].validate(function (valid) {
                if (!valid && that.dialogModalParm.actionType != 'Circulate') {
                    return false
                } else {
                    //alert("aa");
                    //console.log("aa");
                    if (that.dialogModalParm.actionType == 'Circulate') {
                        if (that.circulateUserArrJoin) {
                            if (that.formModel.comment) {
                                ...
                                KStarForm.vmFormData.vmOperation.Comment = '无';
                            }
        ...
        actReSubmit: function () {
            ...
            if (value.Comment) {
                ...
                this.$set(this.formModel, 'comment', '');
            }
        },
        actDraft: function () {
        ...
        actProcPrediction: function () {
            this.moreBtnVisible = false
            KStarForm.ProcPrediction(this, null, true);
        },
        ...
        translatebtnname: function (btncode) {//20191216 翻译按钮名称
            ...
            switch (btncode) {
                ...
                case "Returned"://退回
                    ...
                    //2020-03-18 chenqian “退回”修改为“驳回”
                    btnName = that.$t('KStarForm.Reject');
                    break;
                case "Refused"://拒绝
        ...
        ApplicantSelect: function (loadmore) {
            ...
            axios.post("Platform/SimpleForm/ProcPredictionApprover", data).then(function (res) {
        ...
        SelectChange: function (row, index) {
            var manualUsers = _.find(KStarForm.vmFormData.vmOperation.ManualUsers, { ActivityName: row.ActivityName });
            _.each(row.ApproversUser, function (item) {
                manualUsers.Approvers.push({
                    Approver: item.value,
        ...
        selectApprove: function (row) {
            ...
            this.showdialog2('ApproveUser', true, type, row.ActivityID, orgId, row.ActivityName);
        },
        //打开弹窗
        showdialog2: function (val, multiple, type, activityID, orgId, activityName) {
            ...
            //回显
            var arry = []
            var tempmanualUsers = _.find(KStarForm.vmFormData.vmOperation.ManualUsers, { ActivityName: activityName });
            if (tempmanualUsers) {
                tempmanualUsers.Approvers.forEach(function (item) {
                    arry.push({
                        Type: 'User',
                        Value: item.Approver,
        ...
        userpickCallBack2: function (val, type) {
            ...
            switch (type.clicktype) {
                case 'ApproveUser':
                    var manualUsers = _.find(KStarForm.vmFormData.vmOperation.ManualUsers, { ActivityName: that.rowData.ActivityName });
                    manualUsers.IsChange = true;
                    manualUsers.Approvers = [];
                    if (val.length > 0) {
                        _.each(val, function (item) {
                            var tempModel = _.filter(manualUsers.Approvers, { Approver: item.Value });
                    ...
                    var userAct = _.find(that.procpredictionmodel, function (o) {
                        return o.ActivityName == that.rowData.ActivityName;
                    });
                    var users = [];
                    if (manualUsers.Approvers.length > 0) {
                        _.each(manualUsers.Approvers, function (mu) {
    ...
    computed: {
        isProcessSeer: function () {
            return KStarForm.vmFormData.vmFormInstance.Status < 3;
        }
    }
