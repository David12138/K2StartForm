...
var templateContent = '\
  <div class="module-approval-records">\
        <el-dialog :title="dialogApprovalRecordsTitle" :visible.sync="dialogApprovalRecordsVisible" v-on:cancle="closeApprovalRecordsdialog" @close="closeApprovalRecordsdialog">\
            <div >\
                <el-row class="sub-body custom-reset-styles">\
                    <el-row :class="curActive == \'list\' ? \'displayBlockNotXS\' : \'displayHiddenk\' ">\
                        <el-row class="examination-list" v-for="(item,ind) in stageFormApproval" :key="ind" :class="ind == stageFormApproval.length - 1 ? \'examination-list-last\' : \'\' ">\
                            <el-col>\
                                <i class="radius">{{ ind + 1 }}</i>\
                                <el-row class="activeName">{{ item.ActivityName }}</el-row>\
                                <el-row class="m-content">\
                                    <el-row>\
                                        <el-col v-for="(item2,index) in stageFormApproval[ind].actApprovalHistorys">\
                                            <el-row>\
                                                <el-col :xs="18" :sm="12" :md="12" :lg="12" class="name">\
                                                <el-popover placement="top-start" width="200" trigger="hover" :disabled="disabled" :open-delay="00" v-on:show="getUserInfo(item2.UserAccount)">\
                                                ...
                                                <el-col :xs="6" :sm="12" :md="12" :lg="12" class="action" :class="item2.ActionType == \'Approve\' ? \'font-theme\' : \'\' ">{{ item2.ActionName }}</el-col>\
                                            </el-row>\
                                            <el-row class="visible-xs-only app-gap">\
                                                <el-col :xs="12" :sm="12" :md="12" :lg="12">{{ item2.UserPositionName }}</el-col>\
                                                <el-col :xs="12" :sm="12" :md="12" :lg="12">{{ $t(\'KStarForm.Duration\') }}：{{ FormatDate(item2.EndDate, item2.StartDate) }}</el-col>\
                                            </el-row>\
                    ...
                    <el-table :data="stageFormApprovalTable" border class="m-form-table hidden-xs-only" size="mini" v-if="curActive == \'table\' " :span-method="objectSpanMethod">\
                        ...
                        <el-table-column prop="ActionName" :label="$t(\'KStarForm.Operation\')" width="100px">\
                            <template scope="scope">\
                                <span class="action" :class="addClass(scope.row.ActionType)">{{ scope.row.ActionName }}</span>\
                            </template>\
                        </el-table-column>\
            ...
            <el-tab-pane :label="$t(\'KStarForm.ApprovalRecord\')" name="first">\
                ...
                <el-row class="sub-body">\
                    <el-row :class="curActive == \'list\' ? \'displayBlockNotXS\' : \'displayHiddenk\' ">\
                        <el-row class="examination-list" v-for= "(item,ind) in formapproval" :key = "ind" :class="ind == formapproval.length - 1 ? \'examination-list-last\' : \'\' ">\
                            <el-col>\
                                ...
                                <el-row class="m-content">\
                                    <el-row>\
                                        <el-col v-for="(item2,index) in formapproval[ind].actApprovalHistorys">\
                                            <el-row>\
                                                ...
                                                <el-col :xs="6" :sm="2" :md="2" :lg="2" class="action" :class="addClass(item2.ActionType)">{{ item2.ActionName }}</el-col>\
                                            </el-row>\
                                            <el-row class="visible-xs-only app-gap">\
                                                <el-col :xs="12" :sm="12" :md="12" :lg="12">{{ item2.UserPositionName }}</el-col>\
                    ...
                    <el-table :data="formapprovalTable" border class="m-form-table hidden-xs-only" size="mini" v-if="curActive == \'table\' " :span-method="objectSpanMethod">\
                        ...
                        <el-checkbox v-on:change="loadOtherApprovalRecords(procinstrelationmodel.SecondFormId)" v-if="isShowSecondStage" v-model="approvalchecked"> {{ $t(\'KStarForm.SecondApprovalRecord\') }}</el-checkbox>\
                        <el-checkbox v-on:change="loadOtherApprovalRecords(procinstrelationmodel.FirstFormId)" v-if="isShowFristStage" v-model="approvalchecked"> {{ $t(\'KStarForm.FirstApprovalRecord\') }}</el-checkbox>\
                </el-row>\
                </el-collapse-transition>\
            </el-tab-pane>\
                ...
                <el-collapse-transition>\
                    <div class="sub-body">\
                        <el-row :class="curActive == \'list\' ? \'displayBlockNotXS\' : \'displayHiddenk\'" class="formcirculaterecord-list">\
                            <el-row class="examination-list" v-for= "(item,index) in formcirculaterecord" :key="index">\
                                ...
                                <el-col class="hidden-xs-only">\
                                    ...
                                    <el-row class="activeName">{{ item.ActionName }}</el-row>\
                                    <el-row class="m-content">\
                                        <el-row>\
                                            <el-col>\
                                                <el-row>\
                        ...
                        <el-table :data="formcirculaterecord" border class="m-form-table hidden-xs-only" size="mini" v-if="curActive == \'table\'">\
                            <el-table-column prop="ActionName" :label="$t(\'KStarForm.Type\')"  width="80px"></el-table-column>\
                            <el-table-column prop="UserDisplayName" :label="$t(\'KStarForm.FormCCUser\')">\
                                <template scope="scope">\
                                    <el-popover placement="top-start" width="200" trigger="hover" :disabled="true" :open-delay="800" v-on:show="getUserInfo(scope.row.UserAccount)">\
                                        <el-row v-loading="loading">\
                                            <el-col class="user-info-title" style="font-weight: 700; font-size: 16px">{{userInfo.Name}}</el-col>\
                            ...
                            <el-table-column prop="StartDate" :label="$t(\'KStarForm.Time\')"  width="150px">\
                                <template scope="scope">\
                    ...
                    <div class="sub-body">\
                        <el-row :class="curActive == \'list\' ? \'displayBlockNotXS\' : \'displayHiddenk\'" class="formsubscriberecord-list">\
                            <el-row class="examination-list" v-for= "(item,index) in formsubscriberecord" :key="index">\
                                <el-row class="visible-xs-only">\
                                    <el-col class="actionName" :span="10">{{ item.ActionName }}</el-col>\
                        ...
                        <el-table :data="formsubscriberecord" border class="m-form-table hidden-xs-only" size="mini" v-if="curActive == \'table\'">\
                            <el-table-column prop="ActionName" :label="$t(\'KStarForm.Type\')"></el-table-column>\
...
var moduleApprovalRecords = Vue.extend({
    ...
    props: ['formapprovalmodel', 'formsubscriberecord', 'formcirculaterecord', 'curactivename', 'procinstrelationmodel'],
    data: function () {
        return {
            formapproval: [],
            activeName: 'first',
            ...
            allCheck: false,
            initFormapproval: [],
            dialogApprovalRecordsTitle: '', // 弹出标题
            dialogApprovalRecordsVisible: false, // 是否显示关联流程的审批记录
            stageFormApproval: [], // 弹出数据
            procinstrelation: {},
    ...
    methods: {
        ...
        addClass: function (val) {
            if (val == 'Approve' || val == 'ActionType' || val == 'Communicate' || val == 'ReSubmit' || val == 'Submit' || val == 'CommunicateFeedback' || val == 'PlateformEnd' || val == 'Circulate' || val == 'Subscribe' || val == 'SaveDraft' || val == 'Redirect') {
                // 正向用主题色
                return 'font-theme';
        ...
        loadOtherApprovalRecords: function (formId) {
            this.dialogApprovalRecordsVisible = true;
            //this.dialogApprovalRecordsTitle = title;

            var url = getControllerName() + "/GetRefApprovalHistoryByFormId";
            var _this = this;
            _this.stageFormApprovalTable = []
            var temp = {}
            $post(url, { formId: formId }, function (response) {
                if (response.status == 200) {
                    _this.stageFormApproval = ConvertAppovalRecords(response.data.data);
                    _this.stageFormApproval.map(function (item) {
                        var l = item.actApprovalHistorys.length > 1 ? item.actApprovalHistorys.length : -1
                        item.actApprovalHistorys.map(function (m, n) {
                            temp = m
                            temp.ActivityName = item.ActivityName
                            ...
                            _this.stageFormApprovalTable.push(temp)
                        })
                    })
                }
            }, function (error) {
        ...
        closeApprovalRecordsdialog() {
            this.dialogApprovalRecordsVisible = false;
            this.approvalchecked = false;
        }
    },
    computed: {
        isShowSecondStage: function () {
            if (KStarForm.vmFormData.vmProcInstRelation != null && KStarForm.vmFormData.vmProcInstRelation != undefined && KStarForm.vmFormData.vmFormInstance != null && KStarForm.vmFormData.vmFormInstance != undefined) {
                return (KStarForm.vmFormData.vmProcInstRelation.RecordId > 0 && KStarForm.vmFormData.vmFormInstance.Id == KStarForm.vmFormData.vmProcInstRelation.FirstFormId && KStarForm.vmFormData.vmProcInstRelation.SecondFormId > 0);
            } else {
                return false;
            }
        },
        isShowFristStage: function () {
            if (KStarForm.vmFormData.vmProcInstRelation != null && KStarForm.vmFormData.vmProcInstRelation != undefined && KStarForm.vmFormData.vmFormInstance != null && KStarForm.vmFormData.vmFormInstance != undefined) {
                return (KStarForm.vmFormData.vmProcInstRelation.RecordId > 0 && KStarForm.vmFormData.vmFormInstance.Id == KStarForm.vmFormData.vmProcInstRelation.SecondFormId);
            } else {
                return false;
            }
        }
...
ConvertAppovalRecords = function (approvalRecords) {
    if (approvalRecords) {
        //处理日志
        var formApprovalHistorys = approvalRecords;
        var actApprovalHistoryList = [];
        for (var i = 0; i < formApprovalHistorys.length; i++) {
            var item = formApprovalHistorys[i];
            item.label = i;
            for (var nextIndex = i + 1; nextIndex < formApprovalHistorys.length; nextIndex++) {
                i = nextIndex - 1;
                if (item.ActivityName == formApprovalHistorys[nextIndex].ActivityName) {
                    formApprovalHistorys[nextIndex].label = item.label;
                } else {
                    break;
                }
            }
        }
        var group = _.groupBy(formApprovalHistorys, 'label');
        _.forEach(group, function (el, index) {
            actApprovalHistoryList.push({
                ActivityName: el[0].ActivityName,
                actApprovalHistorys: el
            });
        });
        console.log(actApprovalHistoryList);
        return actApprovalHistoryList;
    }
    return null;
}
// 注册
